...
def test_queue_weekly_summary():
    ...
    expected_start = timestamp(start_of_day(datetime(2021, 4, 5, tzinfo=nzdt)))
    expected_end = timestamp(start_of(SummaryTimespan.WEEKLY, datetime.now(nzdt)))
    ...
    for i in range(len(requests)):
        ...
        assert fromtimestamp(req["startTimestamp"]) == fromtimestamp(expected_start)
